...
void main() {
  test('every build dir is redirected before anything evaluates :app', () {
    // 1.6.0+28 never reached Google Play: file_picker 13 brought the first
    // plugin whose name sorts before "app" (android_file_picker). With the
    // buildDir assignment and evaluationDependsOn(':app') in ONE subprojects
    // block, that plugin forced :app to evaluate before :app's buildDir was
    // redirected, Flutter baked the default R8 rules path under the stale
    // android/app/build, R8 never read its `-dontwarn androidx.**`, and
    // `bundleRelease` failed on missing androidx.window classes. Flutter's
    // own template keeps them in two blocks for exactly this reason.
